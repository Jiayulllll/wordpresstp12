...
class EventM_AJAX {
    ...
    public function save_event_organizer(){
        ...
        foreach($model->organizer_websites as $key => $val) { 
            $model->organizer_websites[$key] = esc_url($val); 
        }
        ...
        if(!empty($posts)){
            ...
            foreach ($posts as $post){
                ...
                if(absint($event->custom_link_enabled) == 1){
                    ...
                    if(!empty(em_global_settings('hide_event_custom_link')) && !is_user_logged_in()){
                        $event->url = add_query_arg('event_id', $event->id, get_permalink(em_global_settings("profile_page")));
                    }
                }
                else{
                    $event->url = add_query_arg('event', $event->id, $event_page);
                }
                $emexpired = '';
                if(em_is_event_expired($event->id)){
                    $emexpired = 'emlist-expired';

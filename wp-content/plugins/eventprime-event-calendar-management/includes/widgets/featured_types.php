...
if (!class_exists('EventM_Featured_Type')){
    ...
    class EventM_Featured_Type extends WP_Widget {
        ...
        function __construct() {
            parent::__construct('eventm_featured_type', "Featured Event Types" , array( 'description' => 'Show list of featured event types.' )
            );
        }
        ...
        public function widget( $args, $instance ) {
            ...
            $html = '<div class="widget widget_featured_events"><div class="widget-content">';
                ...
                $types = $type_service->get_featured_types( $number );
                ...
                if( ! empty( $types ) ){
                    ...
                    foreach ( $types as $type ) {
                        $html .= '<div id="ep-featured-events"  class="ep-mw-wrap">';
                            $typeData = $type_service->load_model_from_db( $type->id );
                            $title = $type->name;
                            $url = $types_page_url = add_query_arg('type', $type->id, get_page_link( $global_settings->event_types ) );

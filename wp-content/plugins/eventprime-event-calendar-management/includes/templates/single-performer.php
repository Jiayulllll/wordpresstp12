...
if ( ! empty( $enable_seo_urls ) ) {
    ...
    if ( ! empty( $post ) && 'em_performer' === $post->post_type && 'trash' !== $post->post_status ) {
        if ( 'draft' === $post->post_status && ! current_user_can( 'manage_options' ) ) {
            esc_html_e( 'This performer is currently in draft state', 'eventprime-event-calendar-management' );
        }
    }
...
if( isset( $atts['event_style'] ) ){
    ...
    $event_display_style = $global_settings->single_performer_event_display_view;
}
if( isset( $atts['event_limit'] ) ){
    $single_performer_event_limit = ( $atts["event_limit"] == 0 || $atts["event_limit"] == '' ) ? EM_PAGINATION_LIMIT : $atts["event_limit"];
}else{
    $single_performer_event_limit = ( $global_settings->single_performer_event_limit == 0 ) ? EM_PAGINATION_LIMIT : $global_settings->single_performer_event_limit;
}
if( isset( $atts['event_cols'] ) ){
    $event_cols = ep_check_column_size( $atts['event_cols'] );
}else{
    $event_cols = ep_check_column_size( $global_settings->single_performer_event_column );
}
if( isset( $atts['load_more'] ) ){
    $load_more = $atts["load_more"];
}else{
    $load_more = $global_settings->single_performer_event_load_more;
}
if( isset( $atts['show_events'] ) ){
    $single_performer_show_events = $atts['show_events'];
}else{
    $single_performer_show_events = $global_settings->single_performer_show_events;
}
if( isset( $atts['hide_past_events'] ) ){
    $hide_past_events = $atts['hide_past_events'];
}else{
    $hide_past_events = $global_settings->single_performer_hide_past_events;
}
/* shortcode end */
...
<div class="ep-performer-page-container">
                            ...
                            <div class="ep-single-box-content">
                                <div>
                                    <div class="ep-single-box-title-info">
                                        <h3 class="ep-single-box-title ep-performer-name" title="<?php echo $performer->name; ?>"><?php echo $performer->name; ?></h3>
                                        <?php if( isset( $performer->role ) ): ?>   
                                            <p class="ep-single-box-designation"><?php echo $performer->role; ?></p>
                                        <?php endif; ?>
                                        <ul class="ep-single-box-details-meta">
                                            ...
                                            <li>
                                                ...
                                                <?php if (  !empty( $performer->performer_phones ) ) echo implode( ', ', $performer->performer_phones ); else _e( 'Not Available', 'eventprime-event-calendar-management' ); ?>
                                            </li>
                                            <li>
                                               <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zm6.93 6h-2.95c-.32-1.25-.78-2.45-1.38-3.56 1.84.63 3.37 1.91 4.33 3.56zM12 4.04c.83 1.2 1.48 2.53 1.91 3.96h-3.82c.43-1.43 1.08-2.76 1.91-3.96zM4.26 14C4.1 13.36 4 12.69 4 12s.1-1.36.26-2h3.38c-.08.66-.14 1.32-.14 2s.06 1.34.14 2H4.26zm.82 2h2.95c.32 1.25.78 2.45 1.38 3.56-1.84-.63-3.37-1.9-4.33-3.56zm2.95-8H5.08c.96-1.66 2.49-2.93 4.33-3.56C8.81 5.55 8.35 6.75 8.03 8zM12 19.96c-.83-1.2-1.48-2.53-1.91-3.96h3.82c-.43 1.43-1.08 2.76-1.91 3.96zM14.34 14H9.66c-.09-.66-.16-1.32-.16-2s.07-1.35.16-2h4.68c.09.65.16 1.32.16 2s-.07 1.34-.16 2zm.25 5.56c.6-1.11 1.06-2.31 1.38-3.56h2.95c-.96 1.65-2.49 2.93-4.33 3.56zM16.36 14c.08-.66.14-1.32.14-2s-.06-1.34-.14-2h3.38c.16.64.26 1.31.26 2s-.1 1.36-.26 2h-3.38z"/></svg>
                                                <?php if ( ! empty( $performer->performer_websites ) ) { 
                                    ...
                                    <?php if ( ! empty( $performer->social_links ) ){ 
                                        echo '<div class="ep-single-box-social">';
                                            if( isset( $performer->social_links->facebook ) )
                                                echo '<a href="'.$performer->social_links->facebook.'" target="_blank" title="Facebook" class="ep-facebook-f"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"> <path d="M504 256C504 119 393 8 256 8S8 119 8 256c0 123.78 90.69 226.38 209.25 245V327.69h-63V256h63v-54.64c0-62.15 37-96.48 93.67-96.48 27.14 0 55.52 4.84 55.52 4.84v61h-31.28c-30.8 0-40.41 19.12-40.41 38.73V256h68.78l-11 71.69h-57.78V501C413.31 482.38 504 379.78 504 256z"/></svg></a></li>';
                                            if( isset( $performer->social_links->instagram ) )
                                                echo '<a href="'.$performer->social_links->instagram.'" target="_blank" title="Instagram" class="ep-instagram"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"/></svg></a>';
                                            if( isset( $performer->social_links->linkedin ) )
                                                echo '<a href="'.$performer->social_links->linkedin.'" target="_blank" title="Linkedin" class="ep-twitter"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M100.28 448H7.4V148.9h92.88zM53.79 108.1C24.09 108.1 0 83.5 0 53.8a53.79 53.79 0 0 1 107.58 0c0 29.7-24.1 54.3-53.79 54.3zM447.9 448h-92.68V302.4c0-34.7-.7-79.2-48.29-79.2-48.29 0-55.69 37.7-55.69 76.7V448h-92.78V148.9h89.08v40.8h1.3c12.4-23.5 42.69-48.3 87.88-48.3 94 0 111.28 61.9 111.28 142.3V448z"/></svg></a>';
                                    ...
                                    <div class="ep-single-box-summery">
                                        <p class="ep-single-box-desc"><?php if (isset($performer->description) && $performer->description !== '') echo $performer->description;
                                    else _e('No description available', 'eventprime-event-calendar-management'); ?></p>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if($single_performer_show_events == 1){
            ...
            <div class="emagic <?php echo $class;?>" id="<?php echo $sec_unique;?>">
                <div id="ep-upcoming-events" class="em_content_area ep-upcoming-events">
                    <div id="post-<?php echo $performer->id; ?>">
                        ...
                        $upcoming_events = $event_service->upcoming_events_for_performer( $performer->id, $args );
                        $event_service->print_upcoming_event_block_for_performers($upcoming_events, $event_args);
                        ?>
                    </div>
                </div>
            </div>
